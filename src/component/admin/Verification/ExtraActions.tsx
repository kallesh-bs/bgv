...
function ExtraActions({ nodata, doc_status_column, doc_column }:{ nodata:boolean, doc_status_column:string, doc_column:string }) {
    ...
    async function handleUpdateDocStatus(userid:string, doc_status:string) {
        ...
        try {
            const { response }:any  = await Helper.post(
                {
    ...
    const handleFileChange = async (event:any, userid:string) => {
