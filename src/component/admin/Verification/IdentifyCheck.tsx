...
const IdentifyCheck = ({ selectOption }:{ selectOption : any}) => {
    ...
    <>
      <div className="w-full pt-10">
        ...
        <div>
          ...
          {data ? data.map((item:any) => <div className="w-full flex items-center justify-between border p-[10px_14px_10px_10px] mt-3">{item.name}<RxCross2 onClick={() => handleFileDelete(empDataById.id, item.url, "identity_check_documents")} className="cursor-pointer" /></div>) : <div className="w-full flex items-center justify-between border p-[10px_14px_10px_10px] mt-3">{'No data'}</div>}
        </div>
      </div>
    </>
  );
